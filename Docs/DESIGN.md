...
1. Add a `UnitBlueprint` to `UnitDatabase`. Stats are expressed at 1★ level 1.
2. Add its id to `UnitDatabase.summonPool` if it should be summonable.
3. Drop `<id>.usdz` and `portrait_<id>.png` into `Resources/`.

There is no step 4. The gacha, the collection, the arena and the battle engine
all read from the blueprint.

## Adding a pantheon

1. Add a case to `Pantheon` with its realm name and accent colour.
...
Leader skills already scope by pantheon, so "Egyptian allies gain 40% HP" is one
line of data.
