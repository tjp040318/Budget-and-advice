...
struct ModelSpec: Codable, Equatable, Sendable {
    /// Base filename without extension, e.g. `zeus`. The loader tries
    /// `zeus.usdz`, then `zeus.scn`, then falls back to a procedural stand-in.
...
enum BattleEnvironment: String, Codable, CaseIterable, Sendable {
    case olympusPeak = "olympus_peak"
    case marbleCourt = "marble_court"
    case stormAltar = "storm_altar"
    case tartarusGate = "tartarus_gate"
    case arenaColosseum = "arena_colosseum"

    var displayName: String {
        switch self {
        case .olympusPeak: return "Peak of Olympus"
        case .marbleCourt: return "Marble Court"
        case .stormAltar: return "Storm Altar"
        case .tartarusGate: return "Gate of Tartarus"
        case .arenaColosseum: return "The Colosseum"
        }
    }
    ...
    var keyLightHex: String {
        switch self {
        case .olympusPeak: return "#FFF3D6"
        case .marbleCourt: return "#F2F0E6"
        case .stormAltar: return "#C8D8FF"
        case .tartarusGate: return "#C08CFF"
        case .arenaColosseum: return "#FFE3B0"
        }
    }

    var fogHex: String {
        switch self {
        case .olympusPeak: return "#8FA8C8"
        case .marbleCourt: return "#B8B4A0"
        case .stormAltar: return "#3C4A70"
        case .tartarusGate: return "#2A1A38"
        case .arenaColosseum: return "#6E5A3C"
        }
    }
}
