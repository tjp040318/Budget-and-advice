...
final class SaveGameTests: XCTestCase {
    ...
    func testNewGameStartsWithZeusEquipped() {
        let save = NewGame.create()
        let player = save.player

        XCTAssertEqual(player.units.count, 1)
        XCTAssertEqual(player.units[0].blueprintID, "zeus")
        XCTAssertEqual(player.units[0].stars, 5)
    ...
    func testPlayerRoundTripsThroughJSON() throws {
        ...
        save.player.campaignProgress["olympus_1"] = 3
        ...
        XCTAssertEqual(restored.player.campaignProgress["olympus_1"], 3)
    ...
    func testEquippingARelicMovesItOffItsPreviousOwner() {
        ...
        let second = Unit(blueprint: UnitDatabase.cerberus, level: 1, stars: 4)
    ...
    func testNoRelicIsEverEquippedTwice() {
        var player = NewGame.create().player
        let extra = Unit(blueprint: UnitDatabase.cerberus, level: 1, stars: 4)
