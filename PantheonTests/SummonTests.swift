...
final class SummonTests: XCTestCase {
    ...
    func testSummonSpendsExactlyOneScrollPerPull() {
        ...
        let results = try? SummonService.summon(
            banner: .duatOpens, count: 10, player: &subject, rng: &rng
        )
    ...
    func testSummonFailsWithoutScrolls() {
        ...
        XCTAssertThrowsError(
            try SummonService.summon(banner: .duatOpens, count: 1, player: &subject, rng: &rng)
        )
    }
    ...
    func testHardPityGuaranteesALegendary() {
        ...
        let cap = Banner.duatOpens.legendaryPity!

        let results = try! SummonService.summon(
            banner: .duatOpens, count: cap, player: &subject, rng: &rng
        )
    ...
    func testPityCounterResetsOnLegendary() {
        ...
        for _ in 0..<200 {
            ...
            let results = try! SummonService.summon(
                banner: .duatOpens, count: 1, player: &subject, rng: &rng
            )
            pullsSinceLast += 1
            if results[0].stars == 5 {
                XCTAssertEqual(subject.summonPity["duat_opens"]?.sinceLegendary, 0)
                pullsSinceLast = 0
            }
            XCTAssertLessThanOrEqual(pullsSinceLast, Banner.duatOpens.legendaryPity!)
        }
    }
    ...
    func testDuplicatesBecomeSkillUps() {
        ...
        _ = try! SummonService.summon(banner: .duatOpens, count: 60, player: &subject, rng: &rng)

        let anubisCopies = subject.units.filter { $0.blueprintID == UnitDatabase.starter.id }
        guard anubisCopies.count > 1 else {
    ...
    func testGachaIsReproducibleFromSeed() {
        func run() -> [String] {
            ...
            let results = try! SummonService.summon(
                banner: .duatOpens, count: 30, player: &subject, rng: &rng
            )
