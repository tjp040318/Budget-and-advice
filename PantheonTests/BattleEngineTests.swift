...
final class BattleEngineTests: XCTestCase {
    ...
    func testSameSeedProducesSameResult() {
        let player = [unit("zeus")]
        let enemies = [unit("cerberus", level: 20, stars: 4), unit("harpy", level: 20, stars: 3)]
    ...
    func testDifferentSeedsDiverge() {
        let player = [unit("zeus")]
        let enemies = [unit("menoetius", level: 30, stars: 5)]
    ...
    func testFasterUnitActsFirst() {
        let fast = unit("harpy", level: 30, stars: 4)   // base SPD 116
        let slow = unit("menoetius", level: 30, stars: 5) // base SPD 92
    ...
    func testBattleAlwaysTerminates() {
        ...
        let a = [unit("bronze_automaton", level: 40, stars: 6), unit("naiad", level: 40, stars: 6)]
        let b = [unit("bronze_automaton", level: 40, stars: 6), unit("naiad", level: 40, stars: 6)]
    ...
    func testDeadUnitsStopActing() {
        let strong = [unit("zeus", level: 60, stars: 6, awakened: true)]
        let weak = [unit("shade_tartarus", level: 1, stars: 1)]
    ...
    func testZeusBasicAttackHitsThreeTimes() {
        let zeus = unit("zeus", level: 40, stars: 6)
        let target = unit("bronze_automaton", level: 40, stars: 6)

        let engine = BattleEngine(playerTeam: [zeus], opponentTeam: [target], mode: .campaign, seed: 5)
        _ = engine.start()

        guard let actorID = engine.awaitingActor else {
            return XCTFail("Zeus is faster and should be awaiting input")
        }

        // The returned stream also carries the opponent's reply, so count only
        // the strikes Zeus himself landed.
        let events = engine.submit(BattleAction(actorID: actorID, skillSlot: 0, targetID: nil))
        let zeusHits = events.filter { event in
            if case .damage(let source, _, _, _, _, _, _, _, _) = event { return source == actorID }
            return false
        }
        XCTAssertEqual(zeusHits.count, 3)
    }

    func testUltimateGoesOnCooldown() {
        let zeus = unit("zeus", level: 40, stars: 6)
        let targets = [unit("harpy", level: 40, stars: 6), unit("naiad", level: 40, stars: 6)]

        let engine = BattleEngine(playerTeam: [zeus], opponentTeam: targets, mode: .campaign, seed: 11)
        _ = engine.start()
        guard let actorID = engine.awaitingActor else { return XCTFail("Zeus should act first or be awaited") }
    ...
    func testAwakenedPassiveIsHiddenUntilAwakened() {
        let base = unit("zeus", level: 40, stars: 6, awakened: false)
        let awakened = unit("zeus", level: 40, stars: 6, awakened: true)

        XCTAssertFalse(base.skills.contains { $0.isPassive })
        XCTAssertTrue(awakened.skills.contains { $0.id == "zeus_passive" })
        XCTAssertGreaterThan(awakened.stats.spd, base.stats.spd)
    }

    func testLeaderSkillAppliesToMatchingPantheonOnly() {
        let zeus = unit("zeus", level: 40, stars: 6)
        let greekAlly = unit("harpy", level: 40, stars: 6) // Greek, so it qualifies

        let engine = BattleEngine(
            playerTeam: [zeus, greekAlly],
            opponentTeam: [unit("naiad", level: 40, stars: 6)],
            mode: .campaign,
            seed: 3
        )
        let boostedAlly = engine.combatants.first { $0.blueprintID == "harpy" && $0.side == .player }
        XCTAssertNotNil(boostedAlly)
        // +33% ATK from the leader skill.
        XCTAssertEqual(boostedAlly!.baseStats.atk, (greekAlly.stats.atk * 1.33).rounded(), accuracy: 2.0)
    }
}
