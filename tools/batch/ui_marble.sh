...
gen ui_button_dark "A single wide, short horizontal plate of DARK SLATE STONE, nearly black, one plate only — no marble, no panels, no dividers, no pattern — three times wider than it is tall, FILLING THE ENTIRE IMAGE edge to edge: the plate's top edge is the top edge of the image, its bottom edge is the bottom edge of the image, and its two ends are the left and right edges of the image, with no background, no margin and nothing else visible. A thin bronze edge runs along the very border of the image all the way round, and plain square ends with no ornament. The entire middle of the plate is flat, even, unornamented dark slate." 1536x448 640 192
